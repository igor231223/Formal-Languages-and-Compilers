...
## Оптимизация 1: устранение одноразовых временных переменных

### Текстовое описание

При генерации IR для составного присваивания number += 1 строится пара тетрад: сначала арифметическая операция с результатом во временную t1, затем копирование t1 в переменную. Если временная используется ровно один раз, пара заменяется одной тетрадой: результат записывается сразу в целевую переменную.

### Преимущества

- Сокращение числа тетрад в теле цикла
- Устранение лишней временной переменной (t1)
- Упрощение IR для последующего анализа и отображения в GUI

### Блок-схема

<img width="760" height="668" alt="image" src="https://github.com/user-attachments/assets/1f5b3054-e60d-4372-b5a3-4ff26587da4e" />
...
## Оптимизация 2: слияние сравнения и условного перехода

### Текстовое описание

Условие while number < 5 в IR задаётся двумя инструкциями: сравнение < number 5 t2 и переход if_true t2 L_body. Если результат сравнения хранится во временной, которая используется только в следующем if_true, обе инструкции заменяются одной - if_< number 5 L_body.


### Преимущества

- Одна инструкция вместо двух в блоке условия
- Устранение временной `t2`
- Более наглядная связь сравнения с переходом (аналог пары `icmp` + `br` в LLVM IR)

### Блок-схема

<img width="1040" height="692" alt="image" src="https://github.com/user-attachments/assets/89ef07b9-2281-4583-a16b-692f5971efb9" />

### Входной и выходной IR (оптимизация 2)

**Входной IR** (результат оптимизации 1, вход для оптимизации 2):
...
## Итоговое сравнение IR

| Этап | Число тетрад | Комментарий |
|------|----------------|-------------|
| Исходный IR | 6 | Две лишние временные: `t1`, `t2` |
| После оптимизации 1 | 5 | Убрана `t1` в теле |
| После оптимизации 2 | 4 | Убрана `t2` в условии |

---

<img width="1919" height="1007" alt="image" src="https://github.com/user-attachments/assets/89ef0f61-d217-42cc-9a99-4a8ae3a617f0" />
