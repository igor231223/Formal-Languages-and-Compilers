...
1. **Лексер:** таблица лексем для корректного выражения и для примера с лексической ошибкой (например `1++2`).
2. **Парсер конструкции repeat-while:** вкладка «Парсер» — без изменений к лаб. 3–5.
3. **Парсер выражения:** вкладка «Тетрады и ПОЛИЗ» — таблица ошибок разбора выражения для некорректной цепочки.

*(Здесь вставьте скриншоты из запущенного `python editor_window.py` по примерам выше.)*
